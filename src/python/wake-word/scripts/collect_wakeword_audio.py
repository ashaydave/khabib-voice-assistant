"""Script to collect data for wake word training..
    ...
    To record customly (usually for recording your own wake words N times).
    use --custom mode
"""
    ...
    try:
        while True:
            ...
            for i in range(int((listener.sample_rate/listener.chunk) * listener.record_seconds)):
                ...
                frames.append(data)
            save_path = os.path.join(args.custom_save_path, "{}.wav".format(index))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--custom_save_path', type=str, default=None, required=False,
                        help='directory to save all the custom 2 second samples. i.e. /to/path/')
    parser.add_argument('--custom', default=False, action='store_true', required=False,
                        help='sets to custom mode')

    args = parser.parse_args()

    if args.custom:
        if args.custom_save_path is None:
            raise Exception('need to set --custom_save_path')
        custom(args)
    else:
